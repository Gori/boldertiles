...
final class ResizeController {
    ...
    func beginResize(at point: CGPoint, frames: [TileFrame], viewportWidth: CGFloat) -> Bool {
        ...
        activeHandle = handle
        dragStartX = point.x
        startWidth = model.items[handle.tileIndex].widthSpec.resolve(viewportWidth: viewportWidth)
        return true
    }
    ...
    func continueResize(at point: CGPoint, viewportWidth: CGFloat) -> Bool {
        ...
        model.items[handle.tileIndex].widthSpec = .fixed(newWidth)
        return true
    }
    ...
    func endResize(viewportWidth: CGFloat) -> Bool {
        ...
        model.items[handle.tileIndex].widthSpec =
            model.items[handle.tileIndex].widthSpec.snappedToPreset(viewportWidth: viewportWidth)

        activeHandle = nil
        return true
    }

    /// Resize the focused item by a delta amount. Used for keyboard resize.
    func resizeFocused(delta: CGFloat, viewportWidth: CGFloat) {
        ...
        let currentWidth = model.items[index].widthSpec.resolve(viewportWidth: viewportWidth)
        let newWidth = max(currentWidth + delta, StripLayout.minimumTileWidth)
        model.items[index].widthSpec = .fixed(newWidth)
    }
